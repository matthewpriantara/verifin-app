    ...
    <div className="mx-auto w-full max-w-6xl px-4 py-8 sm:px-6 lg:py-12">
      ...
      {!auditMode ? (
        ...
        <div className="space-y-4">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.26 }}
            ...
            <div className="flex flex-wrap items-center justify-center gap-2 pt-1">
              <span className="inline-flex items-center gap-1.5 rounded-lg border border-border bg-bg-subtle px-3 py-1.5 text-[12px] text-text-muted">
                <Bell size={13} /> Pemantauan Lowongan <span className="rounded bg-bg-muted px-1.5 py-0.5 text-[10px] font-semibold">Segera hadir</span>
              </span>
              <span className="inline-flex items-center gap-1.5 rounded-lg border border-border bg-bg-subtle px-3 py-1.5 text-[12px] text-text-muted">
                <ChatTeardropText size={13} /> Lapor Komunitas <span className="rounded bg-bg-muted px-1.5 py-0.5 text-[10px] font-semibold">Segera hadir</span>
              </span>
            </div>
        ...
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
          ...
          {/* Raw JSON — collapsible, default tertutup */}
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.28 }}
            className="col-span-4">
            <BentoCard title="Data Mentah Bukti Backend (JSON)" icon={Code}>
              <p className="mb-3 text-[13px] text-text-muted">
                Payload mentah dari backend untuk audit teknis. Tersembunyi secara default.
              </p>
              <div className="space-y-2.5 font-mono text-[11px]">
                {[
                  { icon: Scan,      label: "Entitas Terekstrak", data: entities },
                  { icon: MagnifyingGlass, label: "Output Probe OSINT", data: osint },
                  { icon: ChartBar,  label: "SHAP Evidence Attribution", data: shap },
                  { icon: FileText,  label: "Payload Verifikasi Lengkap", data: report },
                ].map(({ icon: Ic, label, data }) => (
                  <details key={label} className="rounded-xl border border-border bg-bg-subtle p-3">
                    <summary className="flex cursor-pointer items-center justify-between font-bold text-text-primary">
                      <span className="flex items-center gap-2"><Ic size={13} /> {label}</span>
                      <span className="text-[10px] text-text-muted">Buka JSON</span>
                    </summary>
                    <pre className="mt-3 max-h-80 overflow-x-auto rounded-lg border border-border bg-bg-elevated p-3 text-[11px] leading-relaxed text-text-secondary">
